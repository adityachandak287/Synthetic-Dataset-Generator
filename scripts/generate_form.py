...
font_size_min, font_size_max = 8, 14
print("Saving " + str(num_of_images) + " form images.")
for image_counter in tqdm(range(1, num_of_images+1)):
    # yolo_txt_file = output_dir+"\\"+str(image_counter)+".txt"
    yolo_txt_file = os.path.join(output_dir,str(image_counter)+"_a4_form.txt")
    # yolo_txt_file = os.path.join(output_dir,str(image_counter)+"_form.txt")
    f = open(yolo_txt_file, 'w')
    max_sizes = [720, 720, 720, 640, 512]
    ...
    while(True):
        ...
        if max_w-font_w - boundary - curr_pos[0] > 0:
            ...
            spacing = random.choice([[letter_w/2],[letter_w/2],[letter_w],[letter_w*2],[letter_w],[letter_w*2],[letter_w*3]])
            curr_pos[0] += spacing[0]
            # curr_pos[1] += spacing[1]
    ...
    img.save(os.path.join(output_dir,str(image_counter)+"_a4_form.jpg"))
    # img.save(os.path.join(output_dir,str(image_counter)+"_form.jpg"))
    f.close()
    # print("Form "+str(image_counter)+" saved.")
print("Saved " + str(num_of_images) + " form images.")
