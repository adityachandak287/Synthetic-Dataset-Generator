...
aadhar_output_dir = os.path.join(root_dir,"output","medium")
...
num_of_images = 5
font_size_min, font_size_max = 36,36
for image_counter in range(1, num_of_images+1):
    ...
    max_w, max_h = 480, 200#random.choice(max_sizes), random.choice(max_sizes)
    ...
    qr_size = max_h  #random.choice([256,300,300,128])
    ...
    input_text = rstr.xeger(random.choice(sample_input_aadhar))
    font_w, font_h = font.getsize(input_text)[0],font.getsize(input_text)[1] 
    d = ImageDraw.Draw(img)
    num_x, num_y = random.randint(4,qr_x-font_w-20),random.randint(max_h-round(font_h*1.5),max_h-(font_h))
    d.text((num_x, num_y), input_text, font=font, fill=(0, 0, 0))
    
    annotate([num_x, num_y],[font_w, font_h],1,f)
