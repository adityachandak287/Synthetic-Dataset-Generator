...
aadhar_output_dir = os.path.join(root_dir,"output","big")
...
num_of_images = 5
font_size_min, font_size_max = 40,40
for image_counter in range(1, num_of_images+1):
    ...
    max_w, max_h = 480, 280#random.choice(max_sizes), random.choice(max_sizes)
    ...
    qr_size = round(max_h*0.6)  #random.choice([256,300,300,128])
    qr_x = max_w - qr_size #random.randint(0,max_w - qr_size)
    qr_y = 0#random.randint(0,max_h - qr_size)
    ...
    font_bold_path = os.path.join(input_fonts_dir,"Calibri","Calibri_bold.ttf")
    font_bold = ImageFont.truetype(font_bold_path, 48)#random.randint(font_size_min+, font_size_max))
    num_w, num_h = font_bold.getsize(aadhar_num)[0],font_bold.getsize(aadhar_num)[1]
    num_x, num_y = max_w/2-num_w/2, max_h - num_h
    d.text((num_x, num_y), aadhar_num, font=font_bold, fill=(0, 0, 0))
    
    annotate([num_x, num_y],[num_w, num_h],2,f)
